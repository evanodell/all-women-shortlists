...
ggsave("p1_sl1.svg", plot = p_sl1, path="plots", device = "svg",
...
ggsave("p2_lab_keyness_plot.svg", plot = lab_keyness_plot,
...
ggsave("p3_fem_keyness_plot.svg", plot = fem_keyness_plot,
       device = "svg", path = "plots")
...
ggsave("p4_lab_con_keyness_plot.svg", plot = lab_con_keyness_plot,
       device = "svg", path = "plots")
...
ggsave("p5_pro_dfm_key_plot.svg", plot = pro_dfm_key_plot,
       device = "svg", path = "plots")
...
We incorporated the AWS status of speakers and their gender as covariates into our topic model, using all speeches by Labour MPs.
...
ggsave("p6_k_result.svg", plot = p_k_result, path="plots", device = "svg",
       width = 20, height = 20, units = "cm")

p_k_result
```

To identify the best number of topics, we created six topic models with different numbers of topics (_K_). We created models with 30, 45, 60, 80 and 100 topics, and used an algorithm developed by @lee2014c, implemented in the `stm` package [@roberts2018], which resulted in _K_ = 66. As seen in Figure \ref{topic-model-selection}, the algorithm from @lee2014c appears to produce the best result, a topic model with `r formatC(topic_model_k0$settings$dim$K, , big.mark = ",")` topics, across `r formatC(topic_model_k0$settings$dim$N, , big.mark = ",")` speeches with a dictionary of `r formatC(topic_model_k0$settings$dim$V, , big.mark = ",")` words. All models were created using the "spectral" method developed by @arora2013, implemented by @roberts2018. One of the topics -- Topic 66 -- was identified as a possibly topic, but was never the most likely topic in the matrix of number of documents by number of topics -- labelled $\theta$ by @roberts2018, and so while it is included in the model, assignment of single topics to speeches uses the highest $\theta$ for each speech. Other topics are rarely used -- Topic 53, which we labelled "Dispatch Box", only has five topics assigned to it, four from Male MPs and one from an AWS MP.

### Shortlists vs Non-Shortlists

We created a Fruchterman-Reingold [@fruchterman1991] diagram (Figure \ref{k0-network}) to show the connections between different topics. Larger vertices indicate more common topics, and the plot uses a colour scale to indicate the proportion of speeches classed in that topic made by AWS and non-AWS female Labour MPs. The space between vertices indicate the closeness or distance of two topics. E.g. closer vertices represent topics with more overlapping words than more distant topics.

```{r stm-analysis-k0, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.cap="\\label{k0-network}Fruchterman-Reingold plot of Topic Network"}
library(readr)
library(stm)
library(viridis)
library(dplyr)
...
ggsave("p7_network_k0.svg", plot = p_network_k0, path="plots", device = "svg", width = 20, height = 20, units = "cm")

p_network_k0
```


The `stm` package includes an `estimateEffect` function which creates a regression model (Table \ref{estimate-table-k0}) using individual documents (speeches) as individual observations, with the number of documents fitting each topic as the dependent variable and model covariates (AWS status and gender) as independent variables. 
...
ggsave("p8_k0_pyramid.svg", plot = p_k0_pyramid,
...
ggsave("p9_k0_bar.svg", plot = p_k0_bar,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")
...
ggsave("p_10_plot45.svg", plot = plot45,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")
...
ggsave("p_11_plot34.svg", plot = plot34,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")
```
...
ggsave("p_12_plot56.svg", plot = plot56,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")
