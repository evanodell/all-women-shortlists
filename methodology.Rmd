...
1. Re-run liwc assignments, excluding as_speaker==TRUE
...
```{r lab-desc-stats-table, echo=FALSE, results='asis'}
library(knitr)
library(kableExtra)
library(dplyr)

labour_intakes_table <- tibble::tribble(
  ~general_election, ~total_mps, ~total_labour_mps, ~total_female_labour_mps, ~percentage_women_mps, ~newly_elected_mps, ~intake_women, ~percentage_intake_women, ~intake_short_list, ~nominated_short_list,
  1997L, 659L, 418L, 101L, "24%", 177L, 64L, "36%", 35L, 38L,
  2001L, 659L, 412L, 95L, "23%", 38L, 4L, "11%", 0L, 0L,
  2005L, 646L, 355L, 98L, "28%", 40L, 26L, "65%", 23L, 30L,
  2010L, 650L, 258L, 81L, "31%", 64L, 32L, "50%", 28L, 63L,
  2015L, 650L, 232L, 99L, "43%", 49L, 31L, "63%", 31L, 77L
)
# 
...
```{r liwc-creation, eval=FALSE, include=FALSE}
...
```{r short-list-plot-key-variables, echo=FALSE}
library(ggplot2)
...
```{r all-mp-extraction, eval=FALSE, include=FALSE}
## same as above, but this time Conservative vs Labour
library(readr)
library(dplyr)
library(quanteda)
...
```{r all-mp-liwc, eval=FALSE, include=FALSE}
library(readr)
library(dplyr)
library(tidyr)
library(effsize)
...
```{r gender-keyness,  echo=FALSE, fig.height=8}
library(ggplot2)
library(readr)
library(dplyr)
library(quanteda)
...
lab_keyness <- textstat_keyness(lab_dfm_key, measure = c("lr"))
...
```{r short-list-keyness, echo=FALSE, fig.height=8}

lab_dfm_key_fem <- read_rds("lab_dfm_key_fem.rds")

lab_keyness_fem <- textstat_keyness(lab_dfm_key_fem, measure = c("lr"))
...
```{r lab-con-keyness, echo=FALSE, fig.height=8}
library(ggplot2)
library(quanteda)

lab_con_dfm <- read_rds("lab_con_dfm.rds")

lab_con_keyness <- textstat_keyness(lab_con_dfm, measure = c("lr"))
...
lab_con_keyness_plot <- textplot_keyness(lab_con_keyness, n=25, 
                                         color = c("blue", "red")) + 
  #coord_cartesian(xlim=c(-1700,1000)) +
  labs(title = "Keyness in Between Labour and Conservatives",
       x = "likelihood ratio")
...
fem_df2$topic <- recode(fem_df2$topic,
 ...
 "Topic 03" = "Process",
 "Topic 04" = "Business",
 "Topic 05" = "Legislation",
 "Topic 06" = "Tax",
 "Topic 07" = "International",
 "Topic 08" = "Constituents",
 "Topic 09" = "Services",
 "Topic 10" = "Defence",
 "Topic 11" = "Transport",
 "Topic 12" = "Questions",
 "Topic 13" = "Other",
 "Topic 14" = "Children & Families",
 "Topic 15" = "Gender",
 "Topic 16" = "Legal",
 "Topic 17" = "Health",
 "Topic 18" = "Housing",
 "Topic 19" = "Crime",
 "Topic 20" = "Civil Society")

terms_table <- terms_table %>% rename(
  "Education"	=	"Topic 1",
 "Energy" = "Topic 2",
 "Process" = "Topic 3",
 "Business" = "Topic 4",
 "Legislation" = "Topic 5",
 "Tax" = "Topic 6",
 "International" = "Topic 7",
 "Constituents" = "Topic 8",
 "Services"	= "Topic 9",
 "Defence" = "Topic 10",
 "Transport" = "Topic 11",
 "Questions" = "Topic 12",
 "Other" = "Topic 13",
 "Children & Families" = "Topic 14",
 "Gender" = "Topic 15",
 "Legal" = "Topic 16",
 "Health" = "Topic 17",
 "Housing" = "Topic 18",
 "Crime" = "Topic 19",
 "Civil Society" = "Topic 20")
...
```{r tidy-topic-model, include=FALSE}
library(tidytext)
...
speech_classifications <- speech_classifications %>%
  filter(document != "1911783" | topic != 15)

#summary(speech_classifications)

#1911783

# d2 <- lab_speech %>% filter(eo_id == "1206560")
# 
