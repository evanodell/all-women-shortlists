...
kable(lab_con_df, digits = 2, 
      caption = "Effect Sizes for Male and Female MPs, Labour and Conservatives",
      col.names = c("", "Mean", "SD", "Mean", "SD",
                    "Cohen's D", "Magnitude") )  %>%
  kable_styling(latex_options = "striped") %>% 
...
```{r all-parties-gender, include=FALSE}
library(readr)
library(dplyr)
...
all_liwc <- bind_rows(other_liwc, lab_con_liwc)
...
```{r short-list-keyness, echo=FALSE}
...
lab_con_corpus <- read_rds("lab_con_corpus.rds")
...
lab_con_keyness_plot <- textplot_keyness(lab_con_keyness, n=15, 
                                         color = c("blue", "red")) + 
  #coord_cartesian(xlim=c(-1700,1000)) +
...
#x <- filter(beta_df_gamma, document == "1028176")
...
There is more gender distinction on topic
