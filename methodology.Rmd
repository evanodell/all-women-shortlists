...
Data in Table 1 is from House of Commons Library reports [@kelly2016; @audickas2017]. All women short lists were not used by Labour during the 2001 General Election.
...
kable(labour_intakes_table, , format='latex', booktabs = TRUE,
  ...
  kable_styling(bootstrap_options = c("striped", "HOLD_position"), full_width = FALSE)
...
kable(speech_sum_table2, format='latex', booktabs = TRUE, digits = 2, 
...
## Linguistic Inquiry and Word Coun
...
Following @yu2014 drawing on @newman2008 we used the following LIWC categories:
...
library(knitr)
library(kableExtra)
kable(fem_mac_df, format='latex', booktabs = TRUE,  digits = 2, 
...
```{r short-list-plot-key-variables, echo=FALSE, fig.cap="\\label{sl-key-variables}Occurence of selected LIWC terms"}
library(ggplot2)
...
ggsave("p_sl1.svg", plot = p_sl1, path="plots", device = "svg", width = 20, height = 20, units = "cm")
...
library(knitr)
library(kableExtra)
kable(fem_mac_df_sl, format='latex', booktabs = TRUE, digits = 2, 
...
library(knitr)
library(kableExtra)
kable(lab_con_df, format='latex', booktabs = TRUE, digits = 2, 
...
kable(all_party_gender_df, format='latex', booktabs = TRUE, digits = 2, 
...
library(knitr)
library(kableExtra)
kable(pos_df_gender, format='latex', booktabs = TRUE, digits = 2, 
...
library(knitr)
library(kableExtra)
kable(pos_df_sl, format='latex', booktabs = TRUE, digits = 2, 
...
The first implementation used an algorithm developed by @lee2014c, implemented in the `stm` package [@roberts2018], to estimate the number of topics across all speeches made by female Labour MPs, using the "spectral" method developed by @arora2013, implemented by @roberts2018. The resulting topic model has 69 topics, across 81,607 documents and a dictionary of 115,477 words. However, the topic quality with K = 69 is poor, and several topics have poor semantic coherence (see \ref{k69-coherence}).

As seen in the word lists in the [appendix][### Short lists vs Non-Short lists - K69], there is relatively scattershot semantic coherence, although exclusivity is high, when using the 69 topic models suggested by Lee and Mimno's [-@lee2014c] algorithm. We therefore re-ran the analysis, using 30 topic models, which resulted in increased semantic coherence, albeit with slightly lower exclusivity, as illustrated in Figure \ref{k30-coherence}. The lower number of models also makes accurate hand-coding of topics possible.

We created a Fruchterman-Reingold [@fruchterman1991] diagram to show the connections between different topics. Larger vertices indicate more common topics, and the plot uses a colour scale to indicate the proportion of speeches classed in that topic made by AWS and non-AWS female Labour MPs. The space between vertices indicate the closeness of two topics. 

### Short lists vs Non-Short lists - K30
...
test2 <-  selectModel(lab_corpus_fem_stm$documents, 
                    vocab = lab_corpus_fem_stm$vocab, 
                    K = 30, prevalence = ~short_list, 
                    verbose = TRUE, init.type = topic_model_k30)
...
write_rds(corr_topic_k30, "corr_topic_k30.rds")

write_rds(prep_k30, "prep_k30.rds")
```
...
corr_topic_k30 <- read_rds("corr_topic_k30.rds")

prep_k30 <- read_rds("prep_k30.rds")
...
kable(topic_dt_k30_4, format='latex', booktabs = TRUE, longtable = TRUE, 
    ...
    kable_styling(bootstrap_options = c("HOLD_position",  "repeat_header",
...
AWS are -- proportionally -- more likely than non-AWS MPs are on Topics 29 (parliament), 7 (disability) and 24 (media). They are proportionally less likely to mention Topics 15 (justice), 2 (consultations) and 9 (disease). See \ref{k30-topic-proportion} for more details. Surprisingly, AWS MPs are slightly less likely to mention gender issues (Topic 3), although the difference is not statistically significant (see [the appendix][Full topic model estimate summary - K30] for details).
...
p_k30_prop <- ggplot(data = topic_dt_k30_6, 
  ...
  coord_flip() + 
  labs(x = "Topic", 
       y = "Proportion of speeches made by AWS MPs\nrelative to non-AWS MPs") + 
  theme(legend.position = "bottom")
...
kable(topic_words2, format='latex', booktabs = TRUE, longtable = TRUE, 
...
kable(constit_kwic, format='latex', booktabs = TRUE, longtable = TRUE, 
...
### Short lists vs Non-Short lists - K69

The first implementation used an algorithm developed by @lee2014c, implemented in the `stm` package [@roberts2018], to estimate the number of topics across all speeches made by female Labour MPs, using the "spectral" method developed by @arora2013, implemented by @roberts2018. The resulting topic model has 69 topics, across 81,607 documents and a dictionary of 115,477 words. However, the topic quality with K = 69 is poor, and several topics have poor semantic coherence (see \ref{k69-coherence}).

There are several clusters of topics in \ref{k69-network}. For instance, we can see the closeness of Topic 15 (economics and government budgets) and Topic 43 (housing), as both include discussions of budgets and costs, while Topics 23 (bill clauses and admendments) and 16 (education) are very far apart.
...
```{r STM-classification-prep-k69, eval=FALSE, include=FALSE}
library(stm)
library(quanteda)
library(quanteda.corpora)
library(topicmodels)
...
```{r STM-classification-k69, eval=FALSE, include=FALSE}
library(stm)
library(quanteda)
library(quanteda.corpora)
library(topicmodels)
...
write_rds(lab_corpus_man_stm, "lab_corpus_man_stm.rds")

new_man <- alignCorpus(new=lab_corpus_man_stm, old.vocab=topic_model2$vocab)

#new_man <- asSTMCorpus(new_man)

topic_model_man <- fitNewDocuments(model = topic_model2, 
                                   documents = new_man$documents,
                                   newData = new_man$meta, 
                                   prevalence = ~short_list, verbose = TRUE)
write_rds(topic_model_man, "topic_model_man.rds")
write_rds(new_man, "new_man.rds")


summary(topic_model_man)

x <- topicCorr(topic_model_man)
```

```{r corr_creation-k69, eval=FALSE, include=FALSE}
library(readr)
library(stm)
library(viridis)
library(dplyr)
...
corr_topic_k69 <- topicCorr(topic_model2, method = "huge", verbose = FALSE)

prep_k69 <- estimateEffect(1:69 ~ short_list, topic_model2, 
                       meta = lab_corpus_fem_stm$meta, uncertainty = "Global")

write_rds(corr_topic_k69, "corr_topic_k69.rds")

write_rds(prep_k69, "prep_k69.rds")
```

```{r stm-analysis-k69, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.cap="\\label{k69-network}Fruchterman-Reingold plot of K69 Network"}
library(readr)
library(stm)
library(viridis)
library(dplyr)
...
corr_topic_k69 <- read_rds("corr_topic_k69.rds")

prep_k69 <- read_rds("prep_k69.rds")

lab_corpus_fem_stm <- read_rds("lab_corpus_fem_stm.rds")

topic_model2 <- read_rds("stm-topic-model2.rds")

prep_df_k69 <- summary(prep_k69)[[3]]

prep_df_k69 <- as.data.frame(do.call(rbind, prep_df_k69))

prep_df_k69$topic <- rep(1:69, each = 2)

prep_df_k69$type <- row.names(prep_df_k69)

prep_df_k69$type <- gsub("\\.[0-9][0-9]", "", prep_df_k69$type)

prep_df_k69$type <- gsub("\\.[0-9]", "", prep_df_k69$type)

prep_df_k69$type <- gsub("\\.$", "", prep_df_k69$type)

prep_df_k69_coeff <- prep_df_k69 %>% filter(type == "short_listTRUE")

vlabels=NULL
layout=NULL

topics <- 1:nrow(corr_topic_k69$posadj)
  
x <- corr_topic_k69$posadj[topics, topics]
...
plotcord_k69 <- data.frame(layout_with_fr(g))
  
edgelist_k69 <- get.edgelist(g)

#convert to a four column edge data frame with source and destination coordinates
edges_k69 <- data.frame(plotcord_k69[edgelist_k69[,1],], 
                    plotcord_k69[edgelist_k69[,2],])
colnames(edges_k69) <- c("X1","Y1","X2","Y2")

plotcord_k69$topic <- as.numeric(row.names(plotcord_k69))
  
plotcord_k69 <- plotcord_k69 %>% left_join(prep_df_k69_coeff)

lab_corpus_fem_stm$meta$eo_id <- docnames(lab_corpus_fem_stm$documents)

topic_dt_k69 <- make.dt(topic_model2, lab_corpus_fem_stm$meta)

topic_dt_k69$assigned_topic <- colnames(topic_dt_k69[,2:70])[max.col(topic_dt_k69[,2:70],ties.method="random")]

topic_dt_k69$assigned_topic <- as.factor(topic_dt_k69$assigned_topic)

## Reordering x$assigned_topic
topic_dt_k69$assigned_topic <- factor(topic_dt_k69$assigned_topic, 
                           levels=c("Topic1", "Topic2", "Topic3", 
                                    "Topic4", "Topic5", "Topic6",
                                    "Topic7", "Topic8", "Topic9",
                                    "Topic10", "Topic11", "Topic12", 
...
topic_dt_k69_2 <- topic_dt_k69 %>% group_by(assigned_topic) %>%
  summarise(count = n()) %>%
  mutate(freq = count/sum(count) )

plotcord_k69$assigned_topic <- paste0("Topic", plotcord_k69$topic)

plotcord_k69 <- plotcord_k69 %>% left_join(topic_dt_k69_2)

p_network_k69 <- ggplot() + 
  geom_segment(aes(x=X1, y=Y1, xend = X2, yend = Y2),
               data=edges_k69, size = 0.5, colour="grey") +
  geom_point(data = plotcord_k69, aes(X1, X2, colour = Estimate, size = freq)) + 
  geom_text(aes(X1, X2, label=topic), hjust="center", vjust="top",
            size = 5, data = plotcord_k69) + 
  scale_colour_viridis(breaks = c(max(plotcord_k69$Estimate),
                                  min(plotcord_k69$Estimate)), 
                       labels = c("More AWS", "More non-AWS"),
                       name=NULL, option = "plasma") + 
  scale_size(guide = "none") + 
  theme_void() + 
...
ggsave("p_network_k69.svg", plot = p_network_k69, path="plots", device = "svg", width = 20, height = 20, units = "cm")

p_network_k69
```


```{r estimate-table-k69, eval=FALSE, include=FALSE}
library(knitr)
library(kableExtra)
library(dplyr)

row.names(prep_df_k69) <- NULL
prep_df_k69$type <- gsub("X.Intercept", "Intercept", prep_df_k69$type)
prep_df_k69$type <- gsub("short_listTRUE", "Short List", prep_df_k69$type)

prep_df_k69$stars <- case_when(prep_df_k69$`Pr(>|t|)` < 0.001 ~ "***", 
                               prep_df_k69$`Pr(>|t|)` < 0.01 ~ "**",
                               prep_df_k69$`Pr(>|t|)` < 0.05 ~ "*", 
                               TRUE ~ "")


  topics <- prep_k69$topics
  nsim <- 500
  tables <- vector(mode="list", length=length(topics))
  for(i in seq_along(topics)) {
    topic = topics[i]
    sims <- lapply(prep_k69$parameters[[topic]], function(x) stm:::rmvnorm(nsim, x$est, x$vcov))
    sims <- do.call(rbind,sims)
    est<- colMeans(sims)
    se <- sqrt(apply(sims,2, stats::var))
    tval <- est/se
    rdf <- nrow(prep_k69$data) - length(est)
    p <- 2 * stats::pt(abs(tval), rdf, lower.tail = FALSE)
    
    coefficients <- cbind(est, se, tval, p)
    rownames(coefficients) <- attr(prep_k69$parameters[[1]][[1]]$est, "names") 
    colnames(coefficients) <- c("Estimate", "Std. Error", "t value", "Pr(>|t|)")
    tables[[i]] <- coefficients
  }
  out <- list(call=prep_k69$call, topics=topics, tables=tables)
  class(out) <- "lm"
  return(out)
}

kable(prep_df_k69, format='latex', booktabs = TRUE, longtable = TRUE, 
      caption = "Count and Distribution of Topics -- K69",
...
```{r stm-excl-coh-k69, echo=FALSE, fig.cap="\\label{k69-coherence}Coherence of K69 Topic Models"}

library(ggrepel)

sem_test_k69 <- semanticCoherence(topic_model2, lab_corpus_fem_stm$documents)

exc_test_k69 <- exclusivity(topic_model2)

sem_exc_k69 <- tibble::tibble(topic = 1:69, sem_test_k69, exc_test_k69)

p_sem_exc_k69 <- ggplot(sem_exc_k69, aes(y=exc_test_k69, x = sem_test_k69)) + 
  geom_point() + 
  geom_label_repel(aes(label = topic)) +
  coord_cartesian(xlim = c(-200,0), ylim = c(9.4, 10)) + 
  labs(x="Semantic Coherence", y = "Exclusivity")

ggsave("p_sem_exc_k69.svg", plot = p_sem_exc_k69, path="plots", device = "svg", width = 20, height = 20, units = "cm")
...
```{r topic-dt-man-creation-k69, include=FALSE}
library(scales)
library(tidyr)

topic_model_man <- read_rds("topic_model_man.rds")
new_man <- read_rds("new_man.rds")

new_man$meta$eo_id <- docnames(new_man$documents)

topic_man_dt_k69 <- make.dt(topic_model_man, new_man$meta)

topic_man_dt_k69$assigned_topic <- colnames(topic_man_dt_k69[,2:70])[max.col(topic_man_dt_k69[,2:70],ties.method="random")]

topic_man_dt_k69$assigned_topic <- as.factor(topic_man_dt_k69$assigned_topic)

## Reordering x$assigned_topic
topic_man_dt_k69$assigned_topic <- factor(topic_man_dt_k69$assigned_topic, 
                           levels=c("Topic1", "Topic2", "Topic3", 
                                    "Topic4", "Topic5", "Topic6",
                                    "Topic7", "Topic8", "Topic9",
                                    "Topic10", "Topic11", "Topic12", 
...
topic_man_dt_k69_2 <- topic_man_dt_k69 %>% group_by(assigned_topic) %>%
  summarise(man_count = n()) %>%
  mutate(man_freq = man_count/sum(man_count, na.rm = TRUE),
         assigned_topic = gsub("Topic", "Topic ", assigned_topic))
...
```{r topic-dt3-creation-k69, include=FALSE}
topic_dt_k69_3 <- topic_dt_k69 %>% group_by(assigned_topic, short_list) %>%
  summarise(count = n()) %>%
  ungroup() %>%
  spread(key = "short_list", value = "count") %>%
  rename("AWS" = "TRUE", "non_AWS" = "FALSE") %>%
  mutate(AWS_freq = AWS/sum(AWS, na.rm = TRUE),
         ...
         assigned_topic = gsub("Topic", "Topic ", assigned_topic)) %>%
  select(assigned_topic, AWS, AWS_freq, non_AWS, non_AWS_freq) %>%
  left_join(topic_man_dt_k69_2)


topic_dt_k69_4 <- topic_dt_k69_3 %>% 
    mutate(AWS_freq = paste0(round((AWS_freq*100), 2), "%"),
          non_AWS_freq = paste0(round((non_AWS_freq*100), 2), "%"),
          AWS = formatC(AWS, big.mark = ","),
          non_AWS = formatC(non_AWS, big.mark = ","),
...
```{r topic-summary-table-k69, echo=FALSE, results = 'asis'}
library(knitr)
library(kableExtra)

kable(topic_dt_k69_4, format='latex', booktabs = TRUE, longtable = TRUE, 
      caption = "Count and Distribution of Topics -- K69",
      col.names = c("Topic Number", "AWS Speeches", "Percent of AWS Speeches",
                    "Non-AWS Speeches", "Percent of non-AWS Speeches",
                    "Male MP Speeches", "Percent of Male MP Speeches"),
...
```{r k69-topic-pyramid, echo=FALSE, fig.height=9, fig.cap="\\label{k69-topic-pyramid-pllot}K69 Pyramid Chart"}

topic_dt_k69_5 <- topic_dt_k69_3 %>% 
...
topic_dt_k69_5$perc[is.na(topic_dt_k69_5$perc)] <- 0

p_k69_pyramid <- ggplot(data = topic_dt_k69_5, 
                        aes(x = assigned_topic, y = perc, fill = aws_status)) +
  geom_bar(data = filter(topic_dt_k69_5, aws_status == "non_AWS_freq"),
           stat = "identity" ) + 
  geom_bar(data = filter(topic_dt_k69_5, aws_status == "AWS_freq"),
           stat = "identity", aes(y = -perc) ) + 
  geom_bar(data = filter(topic_dt_k69_5, aws_status == "AWS_freq"),
           stat = "identity", aes(color = aws_status),
           alpha = 0, color = "#641A80") + 
...
ggsave("p_k69_pyramid.svg", plot = p_k69_pyramid,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")

p_k69_pyramid
```

```{r k69-topic-bar, echo=FALSE, fig.cap="\\label{k69-topic-bar-plot}K69 Bar Chart"}
p_k69_bar <- ggplot(data = topic_dt_k69_5,
                    aes(x = assigned_topic, y = perc, fill = aws_status)) +
  geom_bar(data = topic_dt_k69_5, stat = "identity", position = "dodge") + 
  scale_y_continuous(breaks = c(0, 0.025, 0.05, 0.075),
                     labels = scales::percent) + 
  scale_fill_viridis_d(labels = c("AWS", "non-AWS"),
                       begin = 0.3, end = 0.7, option = "magma", name = "") + 
...
ggsave("p_k69_bar.svg", plot = p_k69_bar,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")

p_k69_bar
```

```{r top-n-topics-k69, eval=FALSE, include=FALSE}
...
```{r stm-topic-words-k69, message=FALSE, warning=FALSE, include=FALSE}
library(tidyr)
topic_words <- labelTopics(topic_model2, n = 10, frexweight = 0.5)
...
```{r topic-words-table-k69, echo=FALSE, results = 'asis'}
library(knitr)
library(kableExtra)

kable(topic_words2, format='latex', booktabs = TRUE, longtable = TRUE, 
      caption = "Words in topic - K69",
      col.names = c("Topic Number", "Top Ten Words", "Top Ten FREX")) %>%
    kable_styling(bootstrap_options = c("HOLD_position",  "repeat_header",
                                    "striped"), full_width = TRUE ) %>% 
  column_spec(2:3, width = "6cm")
...
## Full topic model summary - K69


```{r topic-model2-summary, echo=FALSE, message=FALSE}
summary(topic_model2)
```

## Full topic model estimate summary - K69

```{r topic-model2-est-summary, echo=FALSE, message=FALSE}
summary(prep_k69)
```
