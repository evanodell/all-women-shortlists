...
kable(labour_intakes_table,  booktabs = TRUE,
  ...
  kable_styling(bootstrap_options = c("basic",  "HOLD_position"), full_width = FALSE)
...
Figure \ref{sl-key-y-since-start} shows changes in the occurences of selected LIWC terms, words-per-sentence, total word count and Flesch–Kincaid grade level, over the course of an MP's career, as measured since the time an MP was first elected. There do not appear to be any notable changes in speaking style over the course of female Labour MPs' careers. Figure \ref{sl-key-date} shows changes in the occurences of the same selected terms from 1997--2017. As in Figure \ref{sl-key-y-since-start}, there do not appear to be any meaningful trends in the use of the selected terms over time.

```{r sl-key-y-since-start, echo=FALSE, fig.cap="\\label{sl-key-y-since-start}Occurence of selected LIWC terms, by time as MP", fig.height=8}
library(ggplot2)
...
```{r sl-key-date, echo=FALSE, fig.cap="\\label{sl-key-date}Occurence of selected LIWC terms, by date", fig.height=8}
library(ggplot2)
library(lubridate)
...
Keyness -- a linguistic measure of the frequency of different words in two groups of texts -- reveals clear gender differences in the most disproportionately common words used by female and male Labour MPs, illustrated in Figure \ref{gender-keyness}.
...
ggsave("p08_network_k0.svg", plot = p_network_k0, path="plots", device = "svg", width = 20, height = 20, units = "cm")
...
kable(prep_df_k0_2, booktabs = TRUE, longtable = TRUE, 
      ...
      col.names = c("", "Estimate", "Standard Error",
                    "t value", "Pr(>|t|)", ""),
      align = c("l","r","r","r","r", "l")) %>%
  kable_styling(bootstrap_options = c("basic", "HOLD_position",  "repeat_header"),
                full_width = TRUE ) %>%
...
The estimate effects in these graphs were extracted using the `tidystm` package by Mikael Poul Johannesson.[^2] Figure \ref{k0-topic-selected-bar-plot} highlights nine topics with different expected proportions between male, AWS and non-AWS Labour MPs, with the error bars representing 95% confidence intervals. See Figure \ref{k0-topic-bar-plot} for a graph of all 66 topics.
...
```{r middle-east-plot, echo=FALSE,fig.cap="\\label{middle-east-plot-validity}Number of Speeches in \"Middle East\" Topic per Year", fig.height=4.5}
...
ggsave("p11_plot45.svg", plot = plot45,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")

```

```{r wales-scotland-plot, echo=FALSE,fig.cap="\\label{wales-scotland-validity}Number of Speeches in \"Wales & Scotland\" Topic per Year", fig.height=4.5}
...
ggsave("p12_plot34.svg", plot = plot34,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")
```


```{r eu-plot, echo=FALSE,fig.cap="\\label{eu-validity}Number of Speeches in \"European Union\" Topic per Year", fig.height=4.5}
...
ggsave("p13_plot56.svg", plot = plot56,
       path="plots", device = "svg", width = 20, height = 20, units = "cm")
...
Figure \ref{k0-theta-boxplot} shows the distribution of $\theta$ scores used to assign overall topics to individual speeches in Table 10, per topic.


```{r theta-boxplot, echo=FALSE, fig.height=7, fig.cap="\\label{k0-theta-boxplot}Theta Values in Topic Assignment"}
library(ggplot2)
