...
1. Run the same STM stuff as below, but with the K30 one
...
```{r short-list-plot-key-variables, echo=FALSE, fig.cap="\\label{sl-key-variables}Occurence of selected LIWC terms", fig.pos = '!h'}
library(ggplot2)
...
ggsave("p_sl1.eps", plot = p_sl1, path="plots", device = "eps", width = 20, height = 20, units = "cm")
...
```{r gender-keyness-plot, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE, fig.cap="\\label{gender-keyness}Keyness between Labour MPs, by Gender", fig.pos = '!h'}
library(ggplot2)
library(readr)
library(dplyr)
library(quanteda)
...
ggsave("lab_keyness_plot.eps", plot = lab_keyness_plot,
       device = "eps", path = "plots")
...
```{r short-list-keyness-plot, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE, fig.cap="\\label{sl-keyness}Keyness between Female Labour MPs, by Selection Process", fig.pos = '!h'}
...
ggsave("fem_keyness_plot.eps", plot = fem_keyness_plot,
       device = "eps", path = "plots")
...
```{r lab-con-keyness, echo=FALSE, fig.height=8, message=FALSE, warning=FALSE, fig.cap="\\label{party-keyness}Keyness between Labour and Conservative MPs", fig.pos = '!h'}
library(ggplot2)
library(quanteda)
...
ggsave("lab_con_keyness_plot.eps", plot = lab_con_keyness_plot,
       device = "eps", path = "plots")
...
```{r bigrams-short-list-keyness, echo=FALSE, message=FALSE, warning=FALSE, fig.height=8, fig.cap="\\label{bigrams-keyness}Bigram Keyness in Female Labour MPs by Selection Process", fig.pos = '!h'}
library(quanteda)
library(readr)
library(ggplot2)
...
ggsave("pro_dfm_key_plot.eps", plot = pro_dfm_key_plot,
       device = "eps", path = "plots")
...
The R package `stm` [@roberts2018] implements a structured topic model (STM) [@roberts2016; @arora2013]. An STM incorporates data about the writer or speaker into the topic classification algorithm. We incorporated the AWS status of speakers into our topic model. 

 <!--We then used `stm`'s implementation of Latent Dirichlet Allocation [@blei2003]-->

We produced two different structured topic model implementations, with different numbers of topics (K). For each implementation, we created Fruchterman-Reingold [@fruchterman1991] diagrams. Larger vertices indicate more common topics, and the plot uses a colour scale to indicate the proportion of speeches classed in that topic made by AWS and non-AWS female Labour MPs. The space between vertices indicate the closeness of two topics.
...
prep_k69 <- estimateEffect(1:30 ~ short_list, topic_model2, 
...
```{r stm-analysis-k69, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.cap="\\label{k69-network}Fruchterman-Reingold plot of K69 Network", fig.pos = '!h'}
library(readr)
library(stm)
library(viridis)
library(dplyr)
...
plotcord <- plotcord %>% left_join(topic_dt_k69_2)
...
ggsave("p_network_k69.eps", plot = p_network_k69, path="plots", device = "eps", width = 20, height = 20, units = "cm")

p_network_k69
```

```{r stm-excl-coh-k69, echo=FALSE, fig.cap="\\label{k69-coherence}Coherence of K69 Topic Models", fig.pos = '!h'}
...
ggsave("p_sem_exc_k69.eps", plot = p_sem_exc_k69, path="plots", device = "eps", width = 20, height = 20, units = "cm")
...
kable(topic_dt_k69_4, booktabs = TRUE, longtable = TRUE, 
...
```{r k69-topic-pyramid, echo=FALSE, fig.height=9, fig.cap="\\label{k69-topic-pyramid-pllot}K69 Pyramid Chart", fig.pos = '!h'}
...
ggsave("p_k69_pyramid.eps", plot = p_k69_pyramid,
       path="plots", device = "eps", width = 20, height = 20, units = "cm")

p_k69_pyramid
```

```{r k69-topic-bar, echo=FALSE, fig.cap="\\label{k69-topic-bar-plot}K69 Bar Chart", fig.pos = '!h'}
...
ggsave("p_k69_bar.eps", plot = p_k69_bar,
       path="plots", device = "eps", width = 20, height = 20, units = "cm")
...
```{r stm-topic-words-k69, message=FALSE, warning=FALSE, include=FALSE, results='asis'}
library(tidyr)
...
However, as seen in the word lists above, there is relatively scattershot semantic coherence, although exclusivity is high, when using 69 topic models. We therefore re-ran the analysis, using 30 topic models, which has resulted in increased semantic coherence, albeit with slightly lower exclusivity.
...
```{r stm-analysis-k30, echo=FALSE, message=FALSE, warning=FALSE, fig.height=7, fig.cap="\\label{k30-network}Fruchterman-Reingold plot of K30 Network", fig.pos = '!h'}
library(readr)
library(stm)
library(viridis)
library(dplyr)
...
p_network_k30 <- ggplot() + 
  ...
  ggsave("p_network_k30.eps", plot = p_network_k30,
         path="plots", device = "eps", width = 20, height = 20, units = "cm")
p_network_k30
```

```{r stm-excl-coh-k30, echo=FALSE, fig.cap="\\label{k30-coherence}Coherence of K30 Topic Models", fig.pos = '!h'}
...
p_sem_exc_k30 <- ggplot(sem_exc_k30, aes(y=exc_test_k30, x = sem_test_k30)) + 
  ...
  ggsave("p_sem_exc_k30.eps", plot = p_sem_exc_k30,
         path="plots", device = "eps", width = 20, height = 20, units = "cm")
p_sem_exc_k30
```
...
```{r k30-topic-pyramid, echo=FALSE, fig.height=9, fig.cap="\\label{k30-topic-pyramid-plot}K30 Pyramid Chart", fig.pos = '!h'}
...
ggsave("p_k30_pyramid.eps", plot = p_k30_pyramid,
       path="plots", device = "eps", width = 20, height = 20, units = "cm")
...
```{r k30-topic-bar, echo=FALSE, fig.cap="\\label{k30-topic-bar-plot}K30 Bar Chart", fig.pos = '!h'}
...
ggsave("p_k30_bar.eps", plot = p_k30_bar,
       path="plots", device = "eps", width = 20, height = 20, units = "cm")
...
```{r constituent-kwic, eval=FALSE, message=FALSE, include=FALSE}
...
constit_kwic <- kwic(lab_corpus_fem_sl, phrase("my constit*"),
  ...
  select(pre:post) %>% sample_frac(0.01)
...
lab_corpus_fem_no_sl <- corpus_subset(lab_corpus, gender == "Female" &
                                short_list == FALSE)

constit_kwic2 <- kwic(lab_corpus_fem_no_sl, phrase("my constit*"),
                     window = 10, valuetype = "glob") %>% 
  select(pre:post) %>% sample_frac(0.01)

kable(constit_kwic2, booktabs = TRUE, longtable = TRUE, 
      col.names = c("Pre", "Keyword", "Post"),
      caption = "A random sample of KWIC's", row.names = FALSE) %>%
    kable_styling(latex_options = c("hold_position",  "repeat_header",
                                    "striped"), full_width = TRUE )
