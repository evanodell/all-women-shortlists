...
kable(labour_intakes_table, escape = FALSE, booktabs = TRUE,
      digits = 2,
...
kable(speech_sum_table2, escape = FALSE, booktabs = TRUE, digits = 2, 
...
Figure \@ref(fig:sl-key-y-since-start) shows changes in the occurences of selected LIWC terms, words-per-sentence, total word count and Flesch–Kincaid grade level, over the course of an MP's career, as measured since the time an MP was first elected. There do not appear to be any notable changes in speaking style over the course of female Labour MPs' careers. Figure \@ref(fig:sl-key-date) shows changes in the occurences of the same selected terms from 1997--2017. As in Figure \@ref(fig:sl-key-y-since-start), there do not appear to be any meaningful trends in the use of the selected terms over time.

```{r sl-key-y-since-start, echo=FALSE, fig.cap="Occurence of selected LIWC terms, by time as MP", fig.height=8}
library(ggplot2)
...
```{r sl-key-date, echo=FALSE, fig.cap="Occurence of selected LIWC terms, by date", fig.height=8}
library(ggplot2)
library(lubridate)
...
library(knitr)
library(kableExtra)
kable(fem_mac_df_sl, escape = FALSE, booktabs = TRUE, digits = 2, 
...
library(knitr)
library(kableExtra)
kable(lab_con_df, escape = FALSE, booktabs = TRUE, digits = 2, 
...
kable(all_party_gender_df, escape = FALSE, booktabs = TRUE, digits = 2, 
...
library(knitr)
library(kableExtra)
kable(pos_df_gender, escape = FALSE, booktabs = TRUE, digits = 2, 
...
library(knitr)
library(kableExtra)
kable(pos_df_sl, escape = FALSE, booktabs = TRUE, digits = 2, 
...
Keyness -- a linguistic measure of the frequency of different words in two groups of texts -- reveals clear gender differences in the most disproportionately common words used by female and male Labour MPs, illustrated in Figure \@ref(fig:gender-keyness).
...
#huxtable::huxtable(prep_df_k0_2)

est_table <- prep_df_k0_2

est_table$topic <- recode(est_table$topic, 
...
library(flextable)
myft <- regulartable(est_table)
myft <- merge_v(myft, ~ topic)
myft <- align(myft, j = c("topic", "type"), 
              align = "left", part = "all") %>% 
...
Figure \@ref(fig:k0-theta-boxplot) shows the distribution of $\theta$ scores used to assign overall topics to individual speeches in Table 10, per topic.
